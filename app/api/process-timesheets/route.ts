...
import { employeeValidator, regionValidator, ValidationStatus } from '@server/validation-system'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const skipDuplicateCheck = searchParams.get('skipDuplicateCheck') === 'true'
    ...
    if (!skipDuplicateCheck) {
      ...
      const entries: Array<[string, File]> = [['site_timesheet', site], ['travel_timesheet', travel], ['overtime_rates', overtime]]
    ...
    const calculateOvertimeHours = (employeeData: Map<string, any>) => {
      for (const [, employee] of Array.from(employeeData.entries())) {
        ...
        for (const [, weekEntries] of Array.from(weeklyEntries.entries())) {
          const totalWeeklyHours = weekEntries.reduce((sum: number, e: any) => sum + e.hours, 0)
    ...
    const pendingMatches: Array<{ input_name: string; line_number?: number; file_type: string; suggestions: Array<{ name: string; score: number; confidence: string }> }> = []
    ...
    const validateAndMatchEmployee = (input: string, lineNumber?: number, fileType: string = 'unknown') => {
      ...
      const needs = matchResult.confidence_score < 95 && matchResult.confidence_score >= 70 && matchResult.suggestions.length > 0
      if (needs) {
        if (!pendingMatches.some(p => p.input_name === input && p.file_type === fileType)) {
          pendingMatches.push({
            ...
            file_type: fileType,
            suggestions: matchResult.suggestions.map((s: any) => ({ name: s.name, score: s.score, confidence: matchResult.confidence }))
          })
        }
      }
      return {
    ...
    if (unknownRegions.size > 0) {
      return NextResponse.json({
        success: false,
        message: `Unknown region(s) found: ${Array.from(unknownRegions).join(', ')}. Please add these as options under the 'Region' tracking category in Xero, then re-run the processing.`,
    ...
    // Travel workbook (first sheet)
    {
      ...
      const data = XLSX.utils.sheet_to_json(sheet, { header: 1 })
      let nameColIndex = -1, dateColIndex = -1, hoursColIndex = -1, regionColIndex = -1
      for (let i = 0; i < data.length; i++) {
        ...
        if (nameColIndex !== -1 && hoursColIndex !== -1) {
          for (let k = i + 1; k < data.length; k++) {
            ...
            const nameStr = String(nameCell).trim()
            const res = validateAndMatchEmployee(nameStr, k + 1, 'travel_timesheet')
            if (!res.match) continue
            const employeeName = res.match
            if (!employeeData.has(employeeName)) {
              employeeData.set(employeeName, { name: employeeName, matchedFrom: nameStr, entries: [], validationNotes: [] })
            }
            let entryDate = '2025-06-02'
            if (travelDate) {
              try {
                const parsed = new Date(travelDate)
                if (!isNaN(parsed.getTime())) entryDate = parsed.toISOString().split('T')[0]
              } catch {}
            }
            const employee = employeeData.get(employeeName)
            const regionName = String(travelRegion).trim()
            const existing = employee.entries.find((e: any) => e.entry_date === entryDate && e.region_name === regionName && e.hour_type === 'REGULAR')
            if (existing) existing.hours += travelHours
            else employee.entries.push({ entry_date: entryDate, region_name: regionName, hours: travelHours, hour_type: 'REGULAR', overtime_rate: null })
            if (!employee.validationNotes.some((n: string) => n.includes('travel time'))) {
    ...
    {
      const sheet = overtimeWb.workbook.Sheets[overtimeWb.workbook.SheetNames[0]]
      const data = XLSX.utils.sheet_to_json(sheet, { header: 1 })
      let nameColIndex = -1, rateColIndex = -1
      for (let i = 0; i < data.length; i++) {
        const row = data[i] as any[]
        if (!row) continue
        for (let j = 0; j < row.length; j++) {
        ...
        if (nameColIndex !== -1 && rateColIndex !== -1) {
          for (let k = i + 1; k < data.length; k++) {
            ...
            if (!nameCell) continue
            const nameStr = String(nameCell).trim()
            const res = validateAndMatchEmployee(nameStr, k + 1, 'overtime_rates')
            if (!res.match) continue
            const employeeName = res.match
            ...
            if (employeeData.has(employeeName)) {
              ...
              employee.validationNotes.push(
                overtimeRate ? `Overtime rate applied: $${overtimeRate.toFixed(2)}.` : 'Overtime rate applied: Standard.'
              )
            }
          }
          break
    ...
    const employeeSummaries = Array.from(employeeData.values()).map((emp: any) => {
      const regularHours = emp.entries.filter((e: any) => e.hour_type === 'REGULAR').reduce((s: number, e: any) => s + e.hours, 0)
      const overtimeHours = emp.entries.filter((e: any) => e.hour_type === 'OVERTIME').reduce((s: number, e: any) => s + e.hours, 0)
      const holidayHours = emp.entries.filter((e: any) => e.hour_type === 'HOLIDAY').reduce((s: number, e: any) => s + e.hours, 0)
